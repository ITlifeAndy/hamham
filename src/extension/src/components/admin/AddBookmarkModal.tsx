...
export const AddBookmarkModal: React.FC<AddBookmarkModalProps> = ({ isOpen, onClose, onBookmarkAdded, pools }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!poolId || !name.trim() || !url.trim()) return;
    
    setLoading(true);
    try {
      await publicBookmarkApi.createBookmark(poolId, name, url);
      onBookmarkAdded();
      onClose();
    } catch (err) {
      alert('創建失敗');
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
      <div className="bg-white rounded-3xl w-full max-w-md overflow-hidden shadow-2xl animate-in fade-in zoom-in duration-200">
        <div className="p-6 border-b border-[#e9eaef] flex items-center justify-between">
          <h3 className="text-xl font-bold text-black font-['Epilogue']">新增書籤</h3>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          <div>
            <label className="block text-sm font-semibold text-slate-600 mb-1 font-['Epilogue']">類別</label>
            <select 
          ...
          <div>
            <label className="block text-sm font-semibold text-slate-600 mb-1 font-['Epilogue']">名稱</label>
            <input 
              className="w-full px-4 py-2 bg-surface-container-low border border-[#e9eaef] rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent outline-none font-body-standard text-sm" 
          ...
          <div>
            <label className="block text-sm font-semibold text-slate-600 mb-1 font-['Epilogue']">URL</label>
            <input 
              className="w-full px-4 py-2 bg-surface-container-low border border-[#e9eaef] rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent outline-none font-body-standard text-sm" 
              placeholder="https://..." 
              type="url"
