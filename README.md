...
4. **發佈與部署**：
   ...
   #Linux 單一檔案
  dotnet publish src/backend/HamHam.Api/HamHam.Api.csproj -c Release -r linux-x64 --self-contained true -p:PublishSingleFile=true -p:PublishTrimmed=false -o ./publish
   ```
5. **運行 API**：
   在 Ubuntu 部署路徑下執行：
