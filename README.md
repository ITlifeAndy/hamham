...
3. **發佈與部署**：
   ...
   #Linux 單一檔案
   dotnet publish src/backend/HamHam.Api/HamHam.Api.csproj -c Release -r linux-x64 --self-contained true -p:PublishSingleFile=true -p:PublishTrimmed=true -o ./publish
   ```
4. **運行 API**：
   在 Ubuntu 部署路徑下執行：
   ```bash
   ...
   2. **貼入以下內容** (請修改 `/var/www/hamham-api` 為您的實際部署路徑)：
      ...
      [Service]
      WorkingDirectory=/var/www/hamham-api
      ExecStart=/usr/bin/dotnet /var/www/hamham-api/HamHam.Api.dll
      Restart=always
      # 如果 API 崩潰，5秒後自動重啟
      RestartSec=5
      KillSignal=SIGINT
